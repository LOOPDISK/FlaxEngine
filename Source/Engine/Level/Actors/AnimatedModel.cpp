...
public:
    ...
    void PostDraw(GPUContext* context, RenderContextBatch& renderContextBatch) override
    {
        ...
        // Runs even with zero bone uploads: dormant skeletons (clean bones) still queue first-draw/LOD-switch
        // dispatches, and dropping them leaks the OutputVersion stamp so the slot never re-skins (invisible mesh).
