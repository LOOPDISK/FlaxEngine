...
namespace FlaxEngine
{
    partial class Renderer
    {
        ...
        /// <summary>
        /// Draws scene lighting with shadows to the output texture (light accumulation buffer).
        /// </summary>
        /// <param name="context">The GPU commands context to use.</param>
        /// <param name="task">Render task to use it's view description and the render buffers.</param>
        ...
        /// <param name="customActors">The custom set of actors to render. If empty, the loaded scenes will be rendered.</param>
        [Unmanaged]
        public static void DrawSceneLighting(GPUContext context, SceneRenderTask task, GPUTexture output, List<Actor> customActors)
        {
            Actor[] temp = null;
            int tempCount = 0;
            if (customActors != null && customActors.Count != 0)
            ...
            Internal_DrawSceneLighting(FlaxEngine.Object.GetUnmanagedPtr(context), FlaxEngine.Object.GetUnmanagedPtr(task), FlaxEngine.Object.GetUnmanagedPtr(output), temp, ref tempCount);
        }
