...
#if USE_INSTANCING
// GPU-driven foliage instance buffers
StructuredBuffer<uint> FoliageVisibleInstances : register(t10);
...
Buffer<float4> FoliageInstanceData : register(t11);
#endif
...
#if USE_INSTANCING
    // GPU-driven foliage: Read from indirect instance buffers
    uint actualInstanceIndex = FoliageVisibleInstances[instanceID];
    object = LoadObject(FoliageInstanceData, actualInstanceIndex);  // Use LoadObject!
    output.Geometry.ObjectIndex = DrawObjectIndex;
#else
    // Traditional rendering: Load object data from standard buffer
    output.Geometry.ObjectIndex = DrawObjectIndex;
    object = LoadObject(ObjectsBuffer, output.Geometry.ObjectIndex);
#endif
...
META_PERMUTATION_1(USE_INSTANCING=1)
META_VS_IN_ELEMENT(POSITION, 0, R32G32B32_FLOAT, 0, 0, PER_VERTEX, 0, true)
float4 VS_Depth(ModelInput_PosOnly input
#if USE_INSTANCING
    , uint instanceID : SV_InstanceID
#endif
) : SV_Position
...
#if USE_INSTANCING
    // GPU-driven foliage: use LoadObject with foliage buffer
    uint actualInstanceIndex = FoliageVisibleInstances[instanceID];
    ObjectData object = LoadObject(FoliageInstanceData, actualInstanceIndex);
    worldPosition = mul(float4(input.Position.xyz, 1), object.WorldMatrix).xyz;
#else
    ObjectData object = LoadObject(ObjectsBuffer, DrawObjectIndex);
    worldPosition = mul(float4(input.Position.xyz, 1), object.WorldMatrix).xyz;
#endif
